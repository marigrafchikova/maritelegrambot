...
class DatabaseManager(object):
    ...
    def __init__(self, path):
        self.conn = psycopg2.connect(path, sslmode="require")
        self.cur = self.conn.cursor()
        # self.cur.execute()
        self.conn.commit()

    def create_tables(self):
        self.query('CREATE TABLE IF NOT EXISTS products (idx text, title text, body text, photo bytea, price int, tag text)')
...
products: idx text, title text, body text, photo bytea, price int, tag text
